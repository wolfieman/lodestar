...
**▶ Live demo → [lodestar.sanyer.org](https://lodestar.sanyer.org)** — chat with Lodestar live (Claude Haiku, streaming). *The hosted demo is a lean Cloudflare Worker build (BM25 retrieval + one streaming Claude call); the full agentic stack — router → tool loop, hybrid RAG (LanceDB), MCP — runs locally and in Docker.*
