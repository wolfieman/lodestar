# IBM "AI Periodic Table" Curriculum → Lodestar Rebuild
...
around the **AI Periodic Table**. This maps each curriculum track to what `src/lodestar/`
actually implements.

| Curriculum track | Concept | Where it lives in Lodestar |
|---|---|---|
...
| **MCP** (expose tools; MCP vs API / RAG) | Model Context Protocol | `mcp_server.py` exposes Lodestar as an MCP server (`retrieve_knowledge`, `get_career_advice`) |
...
"MCP as vendor-convergence" framing: Lodestar is not locked to a single model vendor.
